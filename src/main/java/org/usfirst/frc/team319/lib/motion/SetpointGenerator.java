 ...
 * setpoint as the argument to getSetpoint(), and should only use a measured
 * state directly on the first iteration or if a large disturbance is detected.
 */
public class SetpointGenerator {
    /**
     * A Setpoint is just a MotionState and an additional flag indicating whether
     * this is setpoint achieves the goal (useful for higher-level logic to know
     * that it is now time to do something else).
     */
